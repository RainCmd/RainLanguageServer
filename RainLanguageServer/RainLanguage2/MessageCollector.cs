...
namespace RainLanguageServer.RainLanguage2
{
    ...
    internal readonly struct CompileMessage(TextRange range, ErrorLevel level, string message)
    {
        public readonly TextRange range = range;
        public readonly ErrorLevel level = level;
        public readonly string message = message;
        public readonly List<RelatedInfo> related = [];
        public CompileMessage(IList<TextRange> ranges, ErrorLevel level, string message) : this(new TextRange(ranges[0].start, ranges[^1].end), level, message) { }
    }
    internal class MessageCollector : IEnumerable<CompileMessage>
    {
        ...
        public void Add(TextRange range, ErrorLevel level, string message) => Add(new CompileMessage(range, level, message));
